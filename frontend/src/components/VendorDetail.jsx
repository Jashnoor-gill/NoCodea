...
const VendorDetail = () => {
  ...
  const loadVendor = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/vendors/slug/${slug}`);
      setVendor(response.data.data);
    } catch (error) {
      console.error('Error loading vendor:', error);
      toast.error(t('vendorLoadFailed'));
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">{t('vendorNotFound')}</h1>
            ...
            <div>
              ...
              {vendor?.email && (
                <p className="mt-1 text-sm text-gray-500">
                  {t('contact')}: <a href={`mailto:${vendor.email}`} className="text-blue-600 hover:underline">
                    {vendor.email}
                  </a>
                </p>
              )}
          ...
          <>
            ...
            {products.length > 0 ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-600">{t('noProductsFromVendor')}</p>
              </div>
            )}
