...
const ProductCategory = () => {
  ...
  const loadCategory = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/product-categories/slug/${slug}`);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">{t('categoryNotFound')}</h1>
          <p className="text-gray-600 mb-6">{t('categoryNotFoundMessage')}</p>
          <Link
            to="/products"
